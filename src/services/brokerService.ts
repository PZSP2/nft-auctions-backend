...
export class BrokerService {
    ...
    public static getInstance(): BrokerService {
        if (!BrokerService.instance) {
            BrokerService.instance = new BrokerService(XrpLedgerAdapter.getInstance(), XrpLedgerAdapter.getInstance().getWallet('sEd7Wg2URHoHQjwNPA2iHssrErcUxGF'))
        }
        return BrokerService.instance
    }

    async createAuctionOffers(auctionId: number, nftId: string, price: number, buyerrId: number, sellerId: number): Promise<AuctionOffers> {
        ...
        if (sellerAddress?.wallet_address == null || buyerAddress?.wallet_address == null) {
        ...
        const sellOffer = await this.ledgerAdapter.createSellOffer(sellOfferInfo, this.ledgerAdapter.getWallet(sellerAddress?.wallet_seed ?? ''))
        const buyOffer = await this.ledgerAdapter.createBuyOffer(buyOfferInfo, this.brokerWallet)
    ...
    async getBookersOffers(): Promise<BaseResponse | null> {
        return await this.ledgerAdapter.getOffersByAccount(this.brokerWallet.address)
    }
    ...
    async finalizeAuction(offers: AuctionOffers): Promise<void> {
        ...
        await this.ledgerAdapter.acceptNftOffer(finalizedAuctionDto, this.brokerWallet)
    }
}
