import {
    AccountNFTsRequest,
    AccountNFTsResponse, AccountOffersResponse,
    Client, convertStringToHex, NFTBuyOffersRequest, NFTBuyOffersResponse,
    NFTokenAcceptOffer,
    NFTokenCreateOffer,
    NFTokenMint,
    TxResponse,
...
export default class XrpLedgerAdapter {
    ...
    async createSellOffer(sellOffer: SellOfferInfoModel, wallet: Wallet): Promise<TxResponse> {
    ...
    async createBuyOffer(buyOffer: BuyOfferInfoModel, wallet: Wallet): Promise<TxResponse> {
        const offer: NFTokenCreateOffer = {
    ...
    async getNftOffersByNftId(nftId: string): Promise<NFTBuyOffersResponse | null> {
    ...
    async getOffersByAccount(accountAddress: string): Promise<AccountOffersResponse | null> {
    ...
    acceptNftOffer(offer: AcceptOfferDTO, wallet: Wallet): void {
        ...
        this.client.submitAndWait(transactionBlob, {wallet: wallet}).then( (res) => {
    ...
    async fundWallet(seed: string | null): Promise<Wallet | { wallet: Wallet; balance: number }> {
    ...
    async getAccountNFTsResponse(accountAddress: string): Promise<AccountNFTsResponse | null> {
